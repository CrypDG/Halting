# Halting — On-Demand Professional Driver Marketplace
...
- `apps/admin` — Next.js 15 + Tailwind 4 ops panel, port 3100 (`npm run dev -w @halting/admin`). Client-side Supabase auth; admin actions go through the `admin-actions` edge function.
...
## Supabase
- Project: `halting` (`pybxdufrmnhgneupsssi`), region ap-south-1, org NexaEx. URL/anon key are in `apps/*/.env` and `apps/admin/.env.local`.
- Dev accounts (password `Halting123!`): admin@halting.dev, customer@halting.dev, driver1@halting.dev (LMV/car, approved), driver2@halting.dev (HMV+HTV+PSV → truck/bus/crane/earth mover, approved), driver3@halting.dev (HPMV+PSV+school-bus).
